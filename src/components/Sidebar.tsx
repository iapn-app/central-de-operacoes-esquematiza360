...
export function Sidebar({ collapsed, onToggle }: SidebarProps) {
  ...
  const grupos = filtrarModulos(profile?.role, profile?.email, modulosAtivos);
    ...
    <aside className={`fixed left-0 top-0 z-50 h-screen bg-white border-r border-slate-200 flex flex-col transition-all duration-300 ${collapsed ? "w-[90px]" : "w-[260px]"}`}>
      ...
      <div className="p-3 border-t border-slate-100">
        ...
        <button
          ...
          className={`w-full flex items-center text-slate-600 hover:text-red-600 rounded-xl hover:bg-red-50 text-sm font-medium transition-colors ${
